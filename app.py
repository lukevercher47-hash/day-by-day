...
def load_data():
    ...
    return {
        ...
        "goals": [
            {"text": "Pay dad $80", "due": "today"},
            {"text": "Get oil change $55", "due": "today"},
            {"text": "End the day in profit", "due": "permanent"}
        ],
        "focus_active": False,
        "focus_start": None,
        "focus_session": {
            "income": 0.0, "spending": 0.0, "dashes": 0,
            "miles": 0.0, "active_minutes": 0
        },
        "last_iteration": 7
    }
...
def save_data(data):
    ...
    backup_path = BACKUP_DIR / f"earnings_{today}.json"
...
with st.sidebar:
    ...
    if st.button("✅ Add to Log", type="primary"):
        ...
        st.success(f"Added! Iteration {iter_num} saved")
...
with tab1:
    ...
    st.metric("Active Time", f"{today['active_minutes']//60}h {today['active_minutes']%60}m")
   
    goal = 110.0
    progress = min(today['income'] / goal, 1.0)
    st.write(f"**Progress to ${goal} today**")
